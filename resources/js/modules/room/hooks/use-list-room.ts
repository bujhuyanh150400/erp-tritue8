...
import type { RoomSearchRequest } from '@/modules/room/types';

export const useRoomList = (initialSearch: RoomSearchRequest) => {
    ...
    const [filters, setFilters] = useImmer<
        Partial<RoomSearchRequest['filters']>
    >({
        keyword: '',
        status: undefined,
    });
    ...
    const triggerSearch = useCallback(
        (newFilters: Partial<RoomSearchRequest['filters']>) => {
            const requestData: RoomSearchRequest = {
                filters: newFilters,
                page: 1,
            };

            router.get(listRoom(), requestData, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
                only: ['rooms'],
